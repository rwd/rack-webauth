...
module Rack
  class Webauth
    ...
    class WardenStrategy < Warden::Strategies::Base
      ...
      #
      #     self.finder = lambda { |instance|
      #       mapping.to.find_by_email(instance.webauth.attributes['mail'])
      #     }
      #   end
      #
      ...
      module ClassMethods
        ...
        # else:
        #
        #   Rack::Webauth::WardenStrategy.finder = lambda { |instance|
        #     MyUserModel.find_by_email_address(instance.webauth.attributes['mail'])
        #   }
        #
        # The finder will be evaluated inside the strategy instance,
        # so you have access to "webauth", "env", ...
        def finder
          @finder ||= lambda { |instance|
            Rack::Webauth::User.new(instance.webauth)
          }
        end
      end
